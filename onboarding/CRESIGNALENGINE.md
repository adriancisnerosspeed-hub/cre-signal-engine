...
- **Tier setter UX:** `app/owner/dev/TierSetterPanel.tsx` shows customer-facing names alongside internal slugs — e.g., `Starter (PRO)`, `Analyst (PRO+)`, `Fund / Enterprise (ENTERPRISE)`. After applying a tier change, the panel calls `router.refresh()` so entitlements update immediately without a manual page reload.
