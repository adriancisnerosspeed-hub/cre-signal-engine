...
6. Deterministic severity overrides replace AI-assigned severity with assumption-driven severity for all risk types with numeric proxies (`lib/riskSeverityOverrides.ts`). Overrides now cover all 7 injection risk types using corrected numeric proxies: DebtCostRisk (debt_rate), RefiRisk (debt_rate + hold), ExitCapCompression (exit_cap - cap_rate_in spread), RentGrowthAggressive (rent_growth), ExpenseUnderstated (expense_growth, inverted — low = risk), VacancyUnderstated (vacancy), ConstructionTimingRisk (always Medium). DataMissing is removed entirely when all 8 core assumptions have High confidence.
...
- **Five layers of determinism protection:** (1) input-text-hash cache (7-day TTL), (2) risk_type-based dedup eliminates duplicate risks, (3) **deterministic risk injection** ensures math-warranted risks are always present regardless of AI extraction variance, (4) **expanded severity overrides** force deterministic severity for all risk types with numeric proxies (debt_rate, vacancy, exit_cap spread, rent_growth, expense_growth, hold_period) + DataMissing removal when all 8 core assumptions have High confidence, (5) scoring-input-hash cache guarantees identical normalized inputs reuse exact prior scores.
