/**
 * Build ExportPdfParams from scan id (server-only). Used by export-pdf route and support-bundle.
 */

import type { SupabaseClient } from "@supabase/supabase-js";
import type { ExportPdfParams } from "./exportPdf";
import {
...
export async function getExportPdfPayload(
  ...
  let linksWithRisk: { deal_risk_id: string; risk_type: string; signal_id: string; link_reason: string | null; signal_type: string | null; what_changed: string | null }[] = [];
  if (riskIds.length > 0) {
  ...
  const macroSignals = selectMacroSignalsForPdf({ linksWithRisk, assetType, market });
  ...
  const riskBreakdown = rawBreakdown ? { ...rawBreakdown, stale_scan: staleScan } : null;
